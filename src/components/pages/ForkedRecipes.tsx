...
const ForkedRecipes: React.FC = () => {
  ...
  //passing the values on params, if null we set it to this recipes' id
const OgRecipeId = params.originalRecipeId==='null' ? params.recipeId : params.originalRecipeId 

	useEffect(() => {
		
    // don't need dispatch here cos we have it in the onClick
		// dispatch(getForksById(props));
		//--------------------------
		dispatch(getSingleRecipe(OgRecipeId));
	}, []);
	...
	return (
		<>
			<div className={isNavToggled ? "page-slide-in" : "page-slide-out"}>
				<h2 className="home-sub-header">Original Recipe</h2>
						...
						<div className="recipe-info-wrapper">
							...
							{recipeData.postedOn ? (
								<p className="recipe-facts recipe-el">
									...
									{lengthenDate(recipeData.postedOn)}.
								</p>
							) : null}
							<p className="recipe-facts recipe-el">
								View forks of this recipe below, or click on the image above to
								view the original.
							</p>
						</div>
					</div>
				</div>
				<p className="home-sub-header">Forked Recipes</p>
